...
def ajax_color_request(request):
    ...
    if request.is_ajax() and request.method == 'POST':
        auto_type = Coche.objects.filter(tipo=request.POST.get('tipo', ''))
        colors = auto_type.colores.all() # get all the colors for this type of auto.
    return render_to_response('eleccion.html', {'form': CocheForm})
